@php
    $navId = $outletType === 'bar' ? 'bar-sales-summary-report' : 'lounge-sales-summary-report';
    $routeBase = 'tenant.reports.'.$outletType.'-sales-summary';
    $outletTypeLabel = ucfirst($outletType);
@endphp

<x-layouts.app
    :active-nav="$navId"
    :title="$report['report_title']"
    :subtitle="'Item-level '.$outletTypeLabel.' sales grouped by category'"
>
    ...
    <form method="GET" class="mb-6 space-y-4">
        <x-ui.date-range-filter :from="$from->format('Y-m-d')" :to="$to->format('Y-m-d')">
            <div class="flex flex-col gap-1">
                <label class="text-xs font-medium text-ink-muted">{{ $outletTypeLabel }} outlet</label>
                <select name="outlet_id" class="input-field w-auto min-w-[180px]">
                    <option value="">All {{ strtolower($outletTypeLabel) }} outlets</option>
            ...
            <div class="inline-flex overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <a href="{{ route($routeBase.'.export-excel', $ep) }}"
                ...
                <a href="{{ route($routeBase.'.export-pdf', $ep) }}"
    ...
    <div class="card mb-6 p-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <p class="text-sm font-semibold text-ink">{{ $report['report_title'] }}</p>
        </div>
    </div>

    @if ($report['categories'] === [])
        <div class="card px-6 py-12 text-center text-sm text-ink-muted">
            No {{ strtolower($outletTypeLabel) }} item sales recorded in this period.
        </div>
    @else
