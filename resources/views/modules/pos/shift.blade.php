<x-layouts.app active-nav="restaurant" title="My shift summary" subtitle="Today's orders and sales">
    ...
    <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div class="card p-5 text-center">
            ...
            <p class="mt-1 text-lg font-bold text-emerald-700 sm:text-2xl">{{ $currency }} {{ number_format($summary['cash']) }}</p>
        </div>
        <div class="card p-5 text-center">
            <p class="text-xs font-medium text-ink-muted">Folio posts</p>
            <p class="mt-1 text-lg font-bold text-indigo-700 sm:text-2xl">{{ $currency }} {{ number_format($summary['folio']) }}</p>
        </div>
        <div class="card p-5 text-center">
            <p class="text-xs font-medium text-ink-muted">Total settled</p>
            <p class="mt-1 text-lg font-bold text-ink sm:text-2xl">{{ $currency }} {{ number_format($summary['total_closed']) }}</p>
        </div>
        <div class="card p-5 text-center">
