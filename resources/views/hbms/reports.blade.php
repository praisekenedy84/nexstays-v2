<x-layouts.app active-nav="reports" title="Reports" subtitle="Operational and financial insights">
    ...
    @php
        $features = $features ?? \App\Support\TenantFeatures::reportFlags();
    @endphp

    @if ($features['hotel'])
        @can('manage-reservations')
            <section class="card mb-6 card-pad">
                ...
                <form action="{{ route('tenant.reports.delivery-settings.update') }}" method="POST"
                    ...
                    <div class="flex flex-wrap gap-3 md:col-span-3">
                        <button type="submit" class="btn-primary w-full sm:w-auto">Save email settings</button>
                    </div>
                </form>
                <form action="{{ route('tenant.reports.delivery-settings.send-now') }}" method="POST" class="mt-3">
                    @csrf
                    <button type="submit" class="btn-outline w-full sm:w-auto">Send report now</button>
                </form>
            </section>
        @endcan
    @endif
